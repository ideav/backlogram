...
const sections = [
  {
    h: 'Автоматизация бизнес-процессов: своя разработка и Excel упираются в лимиты',
  ...
  {
    h: 'Для кого: информационные системы и управление бизнес-процессами',
    p: 'Компании, которым нужны внутренние приложения, учёт и автоматизация процессов без долгого цикла заказной разработки: строительство, финансы, логистика, ИТ.',
  },
  {
    h: 'Масштабируемые информационные системы без кода — там, где обычные конструкторы «падают»',
    p: 'Реляционные данные, сотни тысяч записей, права доступа на уровне строк, локальное размещение — сценарии, где Excel, Google Sheets, Airtable и Notion упираются в лимиты. Интеграм в них работает как аналог Excel для корпоративных объёмов: те же таблицы и формулы, но поверх реляционной базы, с правами доступа и историей изменений.',
  },
  {
  ...
  {
    h: 'Готовые типы проектов: описание и анализ бизнес-процессов',
    p: 'CRM, учётные системы, порталы, формы сбора данных, отчётность и интеграции по API (JSON) — от описания бизнес-процесса до работающего приложения.',
  },
  {
