...
| Сумма ₽ по всем датам | 860 мс | **73 мс (12×)** |
| Динамика одного кода | 2 мс | 1 мс |
| Все показатели одной даты | 2 мс | 2 мс |

Обратите внимание: здесь даже агрегат по мере (сумма — она на нижнем уровне) быстрее в 12 раз без всяких накопителей. Причина — размер группы: 400 детей на форму лежат компактно и читаются последовательно, тогда как плоской раскладке нужно 1,8 млн отдельных подъёмов к дате. У транзакций группа была 9,8 — и того же эффекта не хватило, чтобы окупить второй уровень.

Диапазон, который стоит запомнить: **от 7% (широкая запись, узкий разрез) до 40% (узкая запись, ёмкий разрез)** — и знак меняется на минус, если нарушить правило частотности.
