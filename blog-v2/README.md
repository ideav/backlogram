...
5. Подставляет `canonical` фронтматтер на оригинальный URL — пока 301-редиректы со старого блога не настроены, поисковики не наказывают за дубликат.
...
- [ ] Настроить 301-редиректы со старого `blog.ideav.online/2025/.../slug` на `blog.ideav.ru/posts/slug/`. Иначе потеряется индексация.
