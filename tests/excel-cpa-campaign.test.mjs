...
test('целевой кнопки нет в разметке, пока не нажата первая', () => {
  // Весь блок с ценой (а внутри него — кнопка «Записаться») рендерится только
  // при priceOpen. Если однажды кнопку вынесут наружу, схема рушится: автомат,
  // жмущий всё подряд, начнёт приносить оплачиваемые конверсии.
  const priceBlock = landingSource.split('{priceOpen && (')[1]
  assert.ok(priceBlock, 'блок с ценой должен рендериться по условию priceOpen')
  assert.ok(
