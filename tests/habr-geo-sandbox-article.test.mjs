...
test('GEO Habr article is ready to publish without visible draft scaffolding', () => {
  ...
  assert.match(article, /окно возможностей/)
  assert.match(article, /оригинального контента и свежих идей/)
  assert.match(article, /ИИ-чатах/)
  assert.doesNotMatch(article, /жадн|узурпир/i)
